...
def iter_targets(config: Dict[str, Any], region: Optional[str]) -> List[Tuple[str, str, str, int]]:
    ...
    region_filter = (region or "").strip().upper()
    matched_region = False

    # Current schema: environments -> regions -> sub-region -> hosts
    ...
    if isinstance(environments, dict) and environments:
        for env_name, env_obj in environments.items():
            env_regions = (env_obj or {}).get("regions") or {}
            if not isinstance(env_regions, dict):
                continue
            for region_name, region_obj in env_regions.items():
                if region_filter and str(region_name).upper() != region_filter:
                    continue
                matched_region = True
                sub_regions = (region_obj or {}).get("sub-region") or {}
                if not isinstance(sub_regions, dict):
                    continue
                for sub_region_name, sub_obj in sub_regions.items():
                    hosts = (sub_obj or {}).get("hosts") or []
                    for host in hosts:
                        ...
                        for iface in ifaces:
                            scope = f"{env_name}/{region_name}/{sub_region_name}"
                            targets.append((scope, host_id, address + "/" + str(iface), host_port))
    else:
        # Backward-compatible schema: regions -> hosts
        ...
        if isinstance(regions, dict):
            for region_name, region_obj in regions.items():
                ...
                for host in hosts:
                    ...
                    ifaces = host.get("interfaces") or []
                    for iface in ifaces:
                        targets.append((str(region_name), host_id, address + "/" + str(iface), host_port))

    if region_filter and not matched_region:
